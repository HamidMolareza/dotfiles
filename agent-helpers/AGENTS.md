...
- If `memorymcp` appears to expose only read-only tools, use tool discovery with exact tool names such as `store_memory`, `update_memory`, `list_memories`, and `summarize_conversation` before falling back to ad-hoc memory notes.
...
- If the task is C#/.NET-related, or the working directory appears to be a C#/.NET project, and the official JetBrains MCP tools exposed as `rider` are available, first identify whether Rider has the project open and use `rider` for current IDE context when useful, including open editors, solution structure, diagnostics, builds, run configurations, and reformatting.
- For C#/.NET tasks where Rider is open, prefer `rider` as the first source for IDE state that may not be visible from disk alone. Use only capabilities explicitly exposed by the current server; fall back to Rider UI, Git, or disk evidence when no changelist or unsaved-text tool is available.
- Treat the Rider MCP connection as IDE-managed. Do not install `rider-codex-bridge` or hard-code Rider's local endpoint; use Rider's MCP Server Auto-Configure action.
